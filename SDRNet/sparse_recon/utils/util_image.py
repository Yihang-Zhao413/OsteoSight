...
def uint2single(img: np.ndarray) -> np.ndarray:
    """将 uint8 图像 ([0, 255]) 转换为 float32 并归一化到 [0, 1]

    Args:
        img (np.ndarray): 输入图像，要求为 uint8 类型，通道顺序为 HWC（如 RGB 或 BGR）

    Returns:
        np.ndarray: float32 类型，范围 [0, 1]
    """
    if img.dtype != np.uint8:
        raise ValueError("输入图像必须是 uint8 类型")
    return img.astype(np.float32) / 255.0


def single2uint(img: np.ndarray) -> np.ndarray:
    """将 float32 图像 ([0, 1]) 转换为 uint8 ([0, 255])

    Args:
        img (np.ndarray): 输入图像，要求为 float32 类型，通道顺序为 HWC

    Returns:
        np.ndarray: uint8 类型，范围 [0, 255]
    """
    if img.dtype != np.float32:
        raise ValueError("输入图像必须是 float32 类型")
    # 裁剪到 [0, 1] 范围（防止数值溢出）
...
class ImageSpliterTh:
    # 这段代码实现了一个图像分块与重组工具，主要用于将大图像分割为重叠的小块（patch），并支持将处理后的块重新拼合成完整图像。其核心设计目的是在超分辨率（Super-Resolution, SR）等任务中高效处理高分辨率图像，尤其适用于显存有限的场景。
    def __init__(self, im, pch_size, stride, sf=1, extra_bs=1):
        ...
        self.im_ori = im
        self.im_res = np.zeros([height*sf, width*sf,chn], dtype=np.float64)
        self.pixel_count = np.zeros([height*sf, width*sf,chn], dtype=np.float64)
    ...
    def gather(self):
        safe_count = np.where(self.pixel_count == 0, 1.0, self.pixel_count)
        # 结果归一化并转换回uint8
        result = (self.im_res / safe_count).clip(0, 255).astype(np.uint8)
        return result
