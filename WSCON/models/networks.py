...
from WSCON.util.util import ensure_tensor
###############################################################################
# Helper Functions
###############################################################################
...
class GANLoss(nn.Module):
    ...
    def __init__(self, gan_mode, target_real_label=1.0, target_fake_label=0.0):
        ...
        self.register_buffer('real_label', ensure_tensor(target_real_label, dtype=torch.float32))
        self.register_buffer('fake_label', ensure_tensor(target_fake_label, dtype=torch.float32))
...
class PatchSampleF(nn.Module):
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        if self.use_mlp and not self.mlp_init:
            self.create_mlp(feats)  #为每个特征层创建独立的MLP（如mlp_0, mlp_1）;两层线性层+ReLU，实现非线性投影
        for feat_id, feat in enumerate(feats):
            #--------------------------------------------------------
            import matplotlib.pyplot as plt
            #输出中间patch feature
            selected_feat = feat[0]
            # 选择前3个通道
            rgb_feat = selected_feat[:3]  # shape: (3,300,300)
            # 归一化到 [0,1] 范围
            rgb_feat = (rgb_feat - rgb_feat.min()) / (rgb_feat.max() - rgb_feat.min())
            # 转换为HWC格式 (Height, Width, Channel)
            rgb_feat = rgb_feat.permute(1, 2, 0).cpu().detach().numpy()
            plt.imshow(rgb_feat)
            plt.axis('off')
            ...
            feat_reshape = feat.permute(0, 2, 3, 1).flatten(1, 2) #特征重塑 BxCxHxW → Bx(HW)xC
            if num_patches > 0:
                if patch_ids is not None:
                    ...
                    #采样
                    patch_id = np.random.permutation(feat_reshape.shape[1]) #生成随机索引，每个特征层独立采样
                    patch_id = patch_id[:int(min(num_patches, patch_id.shape[0]))]  # .to(patch_ids.device)
                if isinstance(patch_id, torch.Tensor):
                    patch_id = patch_id.clone().detach().to(feat.device).long()
                else:
