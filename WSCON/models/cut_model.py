...
class CUTModel(BaseModel):
    ...
    def forward(self):
        ...
        self.real = torch.cat((self.real_A), dim=0) if self.opt.nce_idt and self.opt.isTrain else self.real_A
    ...
    def compute_D_loss(self):
        ...
        # self.loss_D = (self.loss_D_fake + self.loss_D_real) * 0.5 + self.loss_D_R1
    ...
    def compute_G_loss(self):
        ...
        if self.opt.lambda_identity > 0.0:
            self.loss_idt = torch.nn.functional.l1_loss(self.fake_B, self.real_B) * self.opt.lambda_identity
            # self.loss_idt_2 = self.calculate_NCE_loss(self.real_B, self.fake_B)
            # self.loss_idt = self.loss_idt_1 + self.loss_idt_2 * 0.1
        else:
            self.loss_idt = 0
